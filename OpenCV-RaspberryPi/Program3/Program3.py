...
def runOnWindow(w1, h1, w2, h2, inImage, outName):
    ...
    print("Performing Histogram equalization in Luv space along parameter L...")
    im_Luv = np.array(im_Luv)
    window = im_Luv[h1:h2,w1:w2]
    
    f_W = window[:,:,0].flatten().tolist()
    f_W = [int(round(x)) for x in f_W]        #Round before to get the indecies we'll be counting.
    max_val = max(f_W)+1    #The look up table is through 0-101, so we need length 102 to account for that.
    h_i = [0]*max_val
    print("Computing histogram...")
    for i in range(0, len(f_W)):
      h_i[f_W[i]] += 1              
    sum_of_entries = 0
    f_i = [0]*max_val
    k = 101.0    #Actual value. The 102 is there to account for the fringe case of L = 101
    n = float(len(f_W))
    
    for i in range(0, len(h_i)):
      sum_of_entries += h_i[i]
      f_i[i] = sum_of_entries
    
    f_k = [0.0]*102    #The lookup table. 
    for i in range(0,len(f_i)):
      fi_1 = float(f_i[i-1]) if i > 0 else 0.0
      fi = float(f_i[i])
      j = math.floor( (k*(fi_1 + fi))/(2.0*n))
      f_k[i]=float(j)
    print("Lookup table completed, applying equalization...") 
    out_image = np.zeros((cols, rows, 3)).tolist()
    
    for i in range(0, cols):
        for j in range(0, rows):
            index = clip(int(round(im_Luv[i][j][0])), 0, 100)
            L = f_k[index]
